...
describe('holds', () => {
  it('takes points inside and on the edge, and leaves the ones outside', () => {
    expect(holds(view, 52.5, 13.5)).toBe(true)
    expect(holds(view, 52, 13)).toBe(true)
    expect(holds(view, 51.999, 13.5)).toBe(false)
    expect(holds(view, 52.5, 14.001)).toBe(false)
  })
})
